jQuery(document).ready(function($){
    ...
    $.fn.et_menu = function ( options ) {
        ...
        var methods = {
            ...
            resetExpansion: function(){
                $(parentPlugin).find('>li>.nav-sublist-dropdown').each(function(idx, el){
                    $(el).hide();
                });
                $('.menu-full-width').find('a').removeClass('active');
                $('.menu-full-width .fa').removeClass('fa-caret-up').addClass('fa-caret-down');
            }
        };

        this.find('>li').hover(function (){
            if(!$(this).hasClass('open-by-click') || (!settings.openByClick && $(this).hasClass('open-by-click'))) {
                if(!$(this).hasClass('has-active-ymm')){
        ...
        if(settings.openByClick){
            this.find('>li>a').on('click',function(evt){
            ...
            $(document).on('mouseup',function(e){
                if(!$(parentPlugin).is(e.target) && $(parentPlugin).has(e.target).length === 0){
                    methods.resetExpansion();
                }
            });
        }
    ...
    $('.menu-icon, .close-mobile-nav, .close-sidebar-menu , .header-container.type10 .dropdown-menu > .menu-container > a').click(function(event) {
