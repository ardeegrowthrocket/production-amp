...
jQuery(function($){
    ...
    if( $('.btn-more-fitments').length > 0 ) {
        $('.btn-more-fitments').click( function(e) {
            
            e.preventDefault();

            var _el = $(this);
            var _container = $('#fitment-guide');
            if( _container.length > 0 ) {
                if(_el.hasClass("show-more")) {
                    _container.find('tr.hide').removeClass('hide').addClass('is-show');
                    _el.removeClass('show-more').addClass('show-less').text('Show Less Compatible Models');
                    _container.parent('.fitment-guide-wrapper').slimScroll({
                        height: '350px',
                        width: '100%'
                    });
                } else {
                    _container.find('tr.is-show').removeClass('is-show').addClass('hide');
                    _el.removeClass('show-less').addClass('show-more').text('Show More Compatible Models');
                    _container.parent('.fitment-guide-wrapper').slimScroll({
                        destroy: true
                    }).attr("style","");
                }
