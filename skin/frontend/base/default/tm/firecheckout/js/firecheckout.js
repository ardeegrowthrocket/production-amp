...
FireCheckout.prototype = {
    ...
    save: function(urlSuffix, forceSave) {
        ...
        if (payment.currentMethod) {
            ...
            // bankpayment
            if (!forceSave && payment.currentMethod.indexOf("bankpayment") === 0) {
                payment.save();
                return;
            }
            // bankpayment
        ...
        checkout.setLoadingButton($$('#firecheckout-form button.btn-checkout')[0]);
                ...
                if (el) {
                    ...
                    el.select('input, select').each(function(input) {
                        ...
                        if (input.id && input.id.length && input.id.indexOf(' ') === -1) {
                            ...
                            if (input.hasClassName('qty') || input.name.indexOf('cart_safe') === 0) {
            ...
            if (field.length) {
                field.each(function(_field) {
                    _field.observe('change', function() {
                        ...
                        var billingPostCode = $('billing:postcode').getValue();
                        var billingCountryId = $('billing:country_id').getValue();
                        var billingRegionId = $('billing:region_id').getValue();

                        if(billingCountryId === 'US'){
                            if(billingPostCode === '' || billingCountryId === '' || billingRegionId === ''){
                                return;
                            }
                        }
            ...
            if (field.length) {
                field.each(function(_field) {
                    _field.observe('change', function() {
                        ...
                        var shippingPostCode = $('shipping:postcode').getValue();
                        var shippingCountryId = $('shipping:country_id').getValue();
                        var shippingRegionId = $('shipping:region_id').getValue();

                        if(shippingCountryId === 'US'){
                            if(shippingPostCode === '' || shippingCountryId === '' || shippingRegionId === ''){
                                return;
                            }
                        }
...
ShippingMethod.prototype = {
    ...
    addDelegatedObservers: function () {
        $(document.body).observe('click', function (e) {
            var el = e.findElement('input[name="shipping_method"]');
            ...
            if (storepickupBox) {
                if ('storepickup_storepickup' == el.value) {
                    storepickupBox.show();
                } else {
                    storepickupBox.hide();
                }
            ...
            if (relaypointBox) {
                if ('relaypoint_relaypoint' == el.value) {
                    relaypointBox.show();
                } else {
                    relaypointBox.hide();
                }
            ...
            /* Infostrates TNT */
            if (-1 !== el.value.indexOf('tnt_') && typeof radioCheck !== 'undefined') {
                radioCheck();
            }
            /* Infostrates TNT */
        });
    ...
    addObservers: function() {
        ...
        if (storepickupRadio && $('free-location-box')) {
    ...
    setCheckedRadios: function() {
        $$('[name="shipping_method"]').each(function(el) {
            if (!el.checked && (!el.hasAttribute('checked') || el.readAttribute('checked') === false)) {
                el.checked = false;
                return;
            }
            el.checked = 'checked';
    ...
    validate: function() {
        ...
        for (var i=0; i<methods.length; i++) {
            if (methods[i].checked || $(methods[i]).readAttribute('checked')) {
                return true;
            }
        }
        FC.Messenger.add(
...
Payment.prototype = {
    ...
    init : function () {
        ...
        var parentDiv = $(this.cnt).down('#checkout-payment-method-load dt').up(),
            elements,
            method = null;
        ...
        if (parentDiv) {
            ...
            for (var i=0; i<elements.length; i++) {
                if (elements[i].name=='payment[method]') {
                    if (elements[i].checked || elements[i].readAttribute('checked')) {
...
Review.prototype = {
    ...
    validate: function() {
        ...
        $(checkout.form).select('.checkout-agreements input[type="checkbox"]').each(function(el) {
            if (!el.checked) {
                FC.Messenger.add(
