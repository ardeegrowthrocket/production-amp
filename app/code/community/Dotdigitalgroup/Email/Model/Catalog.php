...
class Dotdigitalgroup_Email_Model_Catalog extends Mage_Core_Model_Abstract
{
    ...
    protected $_productFinalPrice = array();
    ...
    protected function _exportCatalog($storeId)
    {
        ...
        if ($productCollection) {
            ...
            $specialProducts = array(Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE, Mage_Catalog_Model_Product_Type::TYPE_BUNDLE, Mage_Catalog_Model_Product_Type::TYPE_GROUPED);
            foreach ($productCollection as $product) {
                $connectorProduct    = Mage::getModel('ddg_automation/connector_product')
                    ->setProduct($product, $storeId);

                if(!empty($this->_productFinalPrice[$connectorProduct->id]) && !in_array($product->getTypeId(), $specialProducts)){
                    $connectorProduct->price = $this->_productFinalPrice[$connectorProduct->id];
                }
    ...
    protected function _getFinalPrice($store, $productIds)
    {
        Mage::app()->setCurrentStore(Mage_Core_Model_App::ADMIN_STORE_ID);
        $collectionFinalPrice = Mage::getResourceModel('catalog/product_collection')
            ->addAttributeToSelect(array('entity_id','price','final_price'))
            ->setStoreId($store)
            ->addAttributeToFilter('entity_id', array('in' => $productIds))
            ->addStoreFilter()
            ->addFinalPrice();

        if($collectionFinalPrice){
            foreach ($collectionFinalPrice as $product) {
                $this->_productFinalPrice[$product->getId()] = (float)number_format($product->getFinalPrice(), 2, '.', '');
            }
            Mage::log("Total Final Price Sync {$store} :"   . $collectionFinalPrice->getSize(), null, 'connector_api.log', TRUE);
        }
    }
    ...
    protected function _getProductsToExport($store)
    {
        ...
        if ($connectorCollection->getSize()) {
            ...
            $this->_getFinalPrice($store, $productIds);
