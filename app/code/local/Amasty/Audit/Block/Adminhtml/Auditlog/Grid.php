...
class Amasty_Audit_Block_Adminhtml_Auditlog_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('username', array(
            ...
            'index' => 'username',
            'renderer' => 'amaudit/adminhtml_auditlog_renderer_count'
        ));
        ...
        $this->addColumn('status', array(
            ...
            'options' => array(
                ...
                Amasty_Audit_Model_Data::DISABLED => $this->__('Disabled'),
                Amasty_Audit_Model_Data::AUTOLOGOUT => $this->__('Auto logout'),
            ),
    ...
    public function decorateStatus($value, $row, $column, $isExport)
    {
        ...
        switch ($row->getStatus()) {
            case Amasty_Audit_Model_Data::LOGOUT :
            case Amasty_Audit_Model_Data::AUTOLOGOUT :
                $class = 'grid-severity-minor';
                break;
            case Amasty_Audit_Model_Data::UNSUCCESS :
            case Amasty_Audit_Model_Data::LOCKED :
            case Amasty_Audit_Model_Data::DISABLED :
                $class = 'grid-severity-critical';
                break;
            case Amasty_Audit_Model_Data::SUCCESS :
        ...
        if ($value == $this->__('Locked out') && !is_null($timeLock)) {
            $additionals .= '<script type="text/javascript">
                function cleanLock(elem){
                    new Ajax.Request("' . Mage::helper('adminhtml')->getUrl('adminhtml/amaudit_ajax/unlock') . '", {
                    method:"post",
