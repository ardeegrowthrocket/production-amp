...
require(Mage::getBaseDir() . DS . 'lib' . DS . 'Amasty' . DS . 'Auditlog' . DS . 'BrowserDetection.php');
...
class Amasty_Audit_Model_Lockobserver
{
    ...
    public function onAdminSessionUserLoginSuccess($observer)
    {
        ...
        /*
         *   When admin logs in, all other sessions for the same username will be logged out.
        */
        if (Mage::getStoreConfig('amaudit/security/logout')) {
            $otherAdmins = Mage::getModel('amaudit/active')
                ->getCollection()
                ->addFieldToFilter('username', $username);
            if ($otherAdmins->getSize() > 1) {
                Mage::getModel('amaudit/active')->onAdminLogoutNotUnique($otherAdmins);
            }
        }
    }
    ...
    public function onAdminSessionUserLoginFailed($observer)
    {
        ...
        if ($message == 'This account is inactive.') {
            $userData['status'] = Amasty_Audit_Model_Data::DISABLED;
        } else {
    ...
    private function _saveLock($userId)
    {
        ...
        if ($user->hasData()) {
            ...
            if ($count >= Mage::getStoreConfig('amaudit/login/numberFailed') - 1) {
                $user->setData('time_lock', Mage::getModel('core/date')->gmtDate());
            }
            $count++;
