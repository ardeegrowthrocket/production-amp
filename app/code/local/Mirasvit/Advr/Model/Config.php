...
class Mirasvit_Advr_Model_Config
{
    ...
    public function getChartColumnColor($code)
    {
        ...
        if (!$color) {
            $color = $colors[substr(abs(crc32($code)), 0, 1)];
        }
    ...
    public function isDisplayCustomerAddresses()
    {
        return Mage::getStoreConfig('advr/report/customer_report_addresses');
    }


    public function getTimeOfCreatingBy()
    {
        return Mage::getStoreConfig('advr/report/time_of_created_at');
    }

    public function getAdditionalTimeOffset()
    {
        return (int)Mage::getStoreConfig('advr/reporttime/timeoffset');
    }
