...
class Mirasvit_Advr_Model_Postcode extends Mage_Core_Model_Abstract
{
    ...
    public function copyUnknown($verbose = false)
    {
        ...
        do {
            ...
            foreach ($collection as $row) {
                ...
                $post = $this->loadByCode($countryId, $postcode);
                if (!$post) {
                ...
                } elseif ('{"google":[]}' === $post->getOriginal() && 2 !== $post->getUpdated()) {
                    ...
                    $address = array($countryId, $postcode, $region, $city, $street);
    ...
    public function batchUpdateByAddress($verbose = false)
    {
        $limit = 100;
        ...
        do {
            ...
            $collection->getSelect()
                ->where('original NOT LIKE "%google%"')
                ->where('country_id <> "GB"')
                ->where('updated = 2');
            ...
            $toUpdate = array();
            foreach ($collection as $row) {
                $toUpdate[$row->getId()] = $row->getOriginal();
                $lastId = $row->getPostcodeId();
            }

            $resultsGoogle = Mage::helper('advr/geo')->findInGoogle($toUpdate, true);
    ...
    public function batchMerge($verbose = false)
    {
        ...
        do {
            $collection = $this->getCollection()
                ...
                ->setOrder('postcode_id', 'asc');
            $collection->getSelect()
                ->where('original LIKE "%google%" AND original LIKE "%address_components%"')
                ->where('updated IN (0, 2)');
