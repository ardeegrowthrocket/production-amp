    ...
    public function getInterval($code, $timezone = false)
    {
        ...
        switch ($code) {
            ...
            case self::THIS_QUARTER:
                $month = intval(($from->get(Zend_Date::MONTH) - 1) / 3) * 3 + 1;
                $from->setDay(1)
                    ->setMonth($month)
                    ->setTime('00:00:00');
        ...
        $additionalTimeOffset = Mage::getSingleton('advr/config')->getAdditionalTimeOffset();

        $from->addHour($additionalTimeOffset);
        $to->addHour($additionalTimeOffset);
