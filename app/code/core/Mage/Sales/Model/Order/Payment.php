...
class Mage_Sales_Model_Order_Payment extends Mage_Payment_Model_Info
{
    ...
    public function registerCaptureNotification($amount, $skipFraudDetection = false)
    {
        ...
        $disablePaypalIPN = Mage::getModel('core/variable')->loadByCode('disable_paypal_ipn')->getValue('plain');
        if($disablePaypalIPN == 'yes' || $disablePaypalIPN == 1){
            if($state == Mage_Sales_Model_Order::STATE_PROCESSING){
                $msg = $order->getIncrementId() . ' - ' . $state;
                Mage::log($msg, null, 'paypal_ipn_custom.log', true);
                return $this;
            }
        }
