...
class Mage_Shell_Compiler extends Mage_Shell_Abstract{
    ...
    public function run()
    {
        ...
        if($this->getArg('stats')){
            ...
            $filename = 'jeep_products_'.date('m-d-Y_hia') . '.csv';
    ...
    protected function getInfo(){
        ...
        $results = $reader->select()
            ...
            ->where('make = ?', self::JEEP_OPTION_ID)
            ->query()
            ->fetchAll();
        echo sprintf("Number of jeep products: %d \n", count($results));
        echo sprintf("Number of jeep products based on SQL: %d \n",count($this->getData()->fetchAll()));
    }
    protected function getData(){
        ...
        $select = $reader->select()
            ...
            ->where('f.make = ?', self::JEEP_OPTION_ID);
    ...
    {
        return Mage::getBaseDir('var') . DS . 'jau' . DS;
    }
